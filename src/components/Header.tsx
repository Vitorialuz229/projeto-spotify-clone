...
const Header: React.FC = () => {
  ...
  return (
    <header
      className={`flex w-full justify-between items-center px-6 py-3 ${
    ...
    >
      ...
      />
      <a
        className={`cursor-pointer text-2xl transition-transform duration-200 hover:scale-105 ${
      ...
      <div
        ...
        className={`w-[70px] h-[30px] rounded-full relative cursor-pointer transition-all duration-300 ${
          isDarkMode ? "bg-gray-900" : "bg-gray-300"
        }`}
        onClick={toggleTheme}
      >
        <div
          className={`w-[28px] h-[28px] bg-white rounded-full absolute top-[1px] transition-all duration-300 ease-in-out ${
            isDarkMode ? "translate-x-[38px]" : "translate-x-[1px]"
          }`}
        >
          {isDarkMode ? (
